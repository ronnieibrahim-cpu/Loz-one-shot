...
anything to confirm it reproduces byte-identically. **`node tools/check-rippers.mjs` now proves that for all eight**, by re-running each one and comparing — which catches a hand-edit AND a ripper that has quietly stopped being deterministic, the second being the one that does not look like a mistake.
