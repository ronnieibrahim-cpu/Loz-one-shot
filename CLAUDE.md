...
| `node tools/check-music.mjs` | Every track's order references only patterns that exist, no melodic channel holds a note that was never struck, every note (including every note of a vibrato swing and every note of a '+' arpeggio chord) is inside the Game Boy's real frequency range for its channel, and the noise channel carries only percussion and none of the melodic-only options |
