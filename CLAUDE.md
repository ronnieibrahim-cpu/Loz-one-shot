...
| `node tools/check-placement.mjs` | Every entity the room data places can actually BE there — `canOccupy` plus `terrainOk`, with the entity CONSTRUCTED so its own caps answer (`avoidFlags`, `flying`, `swimming`), at all three tide levels. Fails on an entity that fits at NO level, and on two solid entities standing in each other. `--suggest` asks the engine where it could stand instead. A flag sweep cannot do this: it condemns every jellyfish and the raft and misses the sign inside a tree |
