...
- **The overworld is 17 screens across since S157, and a save names screens
  by position.** Tidewatch (Horon Village's size, 0,4,7 to 0,8,8) and Sandpiper
  Row (0,12,8 to 0,14,8) grew by whole COLUMNS, because a grid cannot grow in
  two rows only: every row got the new cells. `tools/oneshot/widen-overworld.mjs`
  is how every reference moved; `widenOverworldX` (src/data/overworld.js) is
  how an OLD SAVE moves, and `tools/test.mjs` proves it. A checker that writes
  its own `OW = 12` or its own list of `0,x,y` keys silently goes on checking
  the old map — read `OVERWORLD_W` and the room data. 42 of the new screens were
  laid down as PLACEHOLDERS (`placeholder: true`: each row is its west
  neighbour's edge tile repeated, so every old crossing still crosses and
  nothing new connects) and are to be authored by hand, region by region.
...
- **Dungeons are being rebuilt at Oracle room size, and a room's size is
  the MAP's.** A map with `cell: [15, 11]` has 15x11 rooms with a wall ring
  drawn by position (`Room.ringArt`), a scrolling camera, and key doors that
  are one door seen from two rooms (`Game.openDoorAt`). `cellTiles(map)`
  (src/world/room.js) is the only place the size lives: six tools once
  computed `sw * 10` and silently stopped at column ten. D1 is rebuilt (S137);
  the recipe for the next one is in `docs/NEXT-SESSION.md` S137.
...
| `node tools/check-strands.mjs` | No room has quietly lost part of its own floor. `check-overworld` floods tile by tile but keys `reached` on the ROOM, so a screen reduced to a four-tile doorway is indistinguishable from one you can walk all of — the tree-crown fix severed The Gyre's whole southern lobe and Drowned Hollow's north doorway, fifteen cells and the corridor between them, with EVERY tool in this table green, `replay` to the pixel and `check-playthrough` included. The lost cells held the sign that teaches the Cleats, and swimming IS the Cleats. This floods on foot with all four gates held open (terrain, not gates), groups the unreached foot-passable cells into regions ACROSS SEAMS — group per-room and a severed corridor reads as two innocent pockets — and diffs `tools/strands-baseline.json`. A baseline, not a zero-assertion: water is legitimately unwalkable and 14 one-cell root pockets inside border treelines are correct. A region that grows or appears with more than one cell fails |
| `node tools/check-dungeon-strands.mjs` | The dungeon equivalent of `check-strands.mjs` above: `walk-dungeons.mjs` has the SAME room-keyed blind spot by construction — its flood walks cell by cell internally but the assertion it reports is "every ROOM reached", so a room reduced to a four-tile doorway would read as fully walkable there too. Shares its flood with `walk-dungeons.mjs` (`tools/lib/dungeon-flood.mjs`) rather than re-deriving which movement verbs each dungeon has been handed by that point — every locked door counted against the dungeon's own key supply, the boss door against its Boss Key, every ledge, every puzzle/wheel/snarl door, the Cleats' swim from D3 on, the Dredge Line's mooring from D6 on — so the two tools cannot silently diverge. Groups unreached floor cells into regions ACROSS ROOM SEAMS AND WARPS and diffs `tools/dungeon-strands-baseline.json`. A baseline, not a zero-assertion: on its first run it found 9 regions, 12 cells, all legitimate — single-tile pockets boxed in by a pot or a wall corner, and one 4-cell floor strip in the Abyssal Keep's Colonnade of the Drowned sealed behind a `dGrate` only the Coastwise Chain's Resonance Rod retracts, a verb this flood does not model (`check-trade.mjs` is what proves that crossing). A region that grows or appears with more than one cell fails |
...
| `node tools/check-anchor.mjs` | A room that claims to need the Tidewright's Anchor cannot be crossed with the conch alone, and can be with one anchor placement — or, for a gate declaring `placements: 2`, cannot be with one and can be with a throw, a recall and a second throw |
...
| `node tools/check-bellows.mjs` | Every room that claims to need the Squall Bellows has a wheel no hand reaches, drowned at the sea the room is played at, freed by one level of cone and by nothing else, from a place you can only stand while it is still drowned — and a sill declared `after` another is sealed at every sea until that one's door opens |
| `node tools/check-reefseed.mjs` | Every room that claims to need the Reefseed grows its stakes on open water no seed can reach at LOW, opens on a snarl no blade but a stake's reaches, and cannot be sealed shut by a pillar the player grew in the wrong place — and every block that must cross a pillar gets to its plate with the stakes grown at LOW and at no sea without them. Its flood knows the conch goes round HIGH -> LOW, so a player who swims out over a pillar at HIGH is set down standing on it |
| `node tools/check-dredge.mjs` | Every room that claims to need the Dredge Line puts its far side across a pit nothing walks, its mooring in reach of one sea and no other, its cache in a floor that only gives up what the sea is covering, and anything it HAULS on a pillar nobody walks to, reached by one sea's cast and no other's — each closure clause proved twice, once at the line's reach and once at the Coilrope's |
...
| `node tools/check-motion.mjs` | Ground enemies stay on the 8px lattice; fliers and swimmers stay off it; and every PORTED enemy (a spec declaring `port`, S151 — it walks as its Oracle of Seasons counterpart does, read from the disassembly) is off the lattice, walks and stands, and moves exactly its cartridge speed along one axis on every frame it moves under its own power |
| `node tools/check-placement.mjs` | Nothing is standing inside anything. Builds every room in the real engine and asks `canOccupy` **and** `terrainOk` — the engine's own pair, with no caps passed, so each entity's own nature answers — whether every placed entity can be where the room data put it, at any tide. A sign inside a tree, a trader inside a bush and a boss standing on a one-way ledge were all invisible to every other tool here, because a placed entity spawns, updates and draws identically whether or not there is a rock in the same square. `--json` emits the list with a suggested tile; the suggestion is a legal tile, never a good one |
| `node tools/check-ground.mjs` | Nothing is standing on ground its screen does not have. A prop is a transparent tile that NAMES the ground drawn beneath it — `rock` says `grass` — and that name is a property of the tile table, not of the place the tile was put, so a rock dropped on a reef brings a lawn with it and lands as a hard green square. Calls `Room.underGround` (R4) for every prop cell at every tide level and compares the answer's PALETTE against the grounds the screen actually has; `sand` and `sandRipple` are one material and pass, `grass` on a screen of `grassDark` and `mud` does not. 274 cells failed it the day it was written. Also that no room warp sits on a tile a 32x32 object may overhang — a room's `warps` list is a doorway that need not carry `F.WARP`, so `quadMayCover` says yes to it and a canopy would swallow the way in; that a 32x32 object overhangs the SAME cells at every tide, because `quadMayCover` refuses an animated cell and a tide tile is animated at some levels and not others, so 66 cells had the woods growing and shrinking as the conch was sounded; and that no decorative prop stands three in a straight line — sixteen screens laid their rocks out as `oooo`, five of them twice with an `o..o` between, which is a fence in open sand and reads from inside as Link standing on a field of rocks. Gates, ledges and trees are exempt, by flag rather than by name; and that no 32x32 object is cut short by plain ground — a refused quadrant is simply not drawn, so a tree beside a flower bed came out with a dead straight edge down its middle, 146 of 536 tree blocks incomplete; and that no placed OBJECT — a sign, a pickup, a chest — stands inside an overhang, a treeline being two tiles deep on screen and one in the data. People are reported, not asserted: an NPC stands beside the thing it belongs to, and the automatic move put the scrimshander in the middle of the village's only straight route west |
...
| `node tools/check-camera.mjs` | The camera is pinned at 0,0 in every room no bigger than the view (181 of 359 at S157), never leaves the room, never moves more than `CAM_MAX_SPEED` in a frame, is always whole-pixel, ignores a player inside the deadzone, and actually follows in the 178 rooms that are bigger than the view |
| `node tools/check-wide-rooms.mjs` | Every multi-screen room fills its authored grid (a short row silently becomes void), fits on its map, owns its cells alone, resolves every covered cell back to itself, is crossable at every internal seam by SOME verb, and never neighbours itself |
| `node tools/check-feel.mjs` | Every constant in `feel.js` has a comment with a unit and a provenance word, and **anything claiming `measured` must name the reference it was frame-stepped from**. `measured` has a specific meaning and inflating it destroys the file permanently |
| `node tools/check-text.mjs` | Every character the game can display has a glyph. `decode` falls back to `GLYPHS['?']`, so a missing glyph is not a gap or a crash — it is a QUESTION MARK that reads as authored punctuation. Six Essence title cards said "I ? the Shallow Bell" for the project's whole life |
| `node tools/shoot-cutscene.mjs` | Photographs each cutscene mid-scene, `--nereth` proves `nerethIntro` actually fires by reaching the throne room, and `--ending` proves claiming the sixth Essence actually chains into `ending` (it did not: nothing in the game ever called `startCutscene('ending')` until S43 wired `Game.claimEssence` to queue it) |
| `node tools/watch-cutscenes.mjs` | Plays all 13 cutscenes END TO END at 60fps, turning each dialogue page after a human reading time rather than instantly, and writes a beat-by-beat timeline (`--strips` adds a filmstrip per scene). Proves no scene hangs, and proves the ONE decidable thing about pacing: no caption is ever gone before its own words can be read. **Pacing itself is not assertable (§4.2)** — this ends with a person watching, and the timeline is what they read first |
| `node tools/check-dialogue.mjs` | Every dialogue id the world asks for is written, and every id written is asked for — `startDialogue` shows an EMPTY BOX on a miss and only warns to a console nobody reads. Also constructs each two-state townsperson and drives the real `NPC.interact` either side of its threshold, so a second line that is wired but unreachable fails |
| `node tools/check-sfx.mjs` | Every sfx name a call site or a data table can reach is actually defined — `Audio.sfx` returns silently on a miss, so a typo or a sound nobody authored is a verb that makes no noise and NOTHING else notices. Six such calls had survived six sessions. Also reports a sound that is defined and played by nothing |
| `node tools/check-music.mjs` | Every track's order references only patterns that exist, no melodic channel holds a note that was never struck, every note (including every note of a vibrato swing and every note of a '+' arpeggio chord) is inside the Game Boy's real frequency range for its channel, and the noise channel carries only percussion and none of the melodic-only options. Also that a track's `intro` — its non-looping lead-in — names real patterns, shares none with the loop, and is actually SPENT EXACTLY ONCE by the engine, which it proves by driving `Audio._scheduleRow` for two loops rather than by modelling where the wrap falls |
| `node tools/check-audio-render.mjs` | The music engine schedules the exact same Web Audio instructions it always did for a track that asks for none of vibrato/echo/arpeggio — a regression here would be silent otherwise. Traces `Audio._scheduleRow` against a mock context in plain Node rather than hashing rendered samples, because real Web Audio rendering is not bit-reproducible across script contexts even for identical code |
...
| `node tools/check-rippers.mjs` | Every generated module is what its ripper actually emits. CLAUDE.md has always said extraction lands in a generated file and never to hand-edit one — and NOTHING ENFORCED IT: `check-tilesets` verifies exactly one ripper (`rip-dungeon-maps.py`, via its own `--verify`), and the eight that emit the modules the game draws from were on the honour system. Re-runs each and compares byte for byte, which catches two different faults: a generated file edited by hand, whose edit the next regeneration throws away silently; and A RIPPER THAT HAS STOPPED BEING DETERMINISTIC, which is the worse one because nothing about it looks like a mistake — `rip-hud.py`'s own header records that its rip was three pixels unstable across runs before it stopped using ripkit's nearest-colour search, and a checker that only diffed the committed file against itself could never see that. Restores the file from git on a mismatch, so a red run does not also leave a dirty tree, and sweeps `tools/` for a `rip-*.py` its table does not name. SKIPs (exit 2) without Pillow rather than passing. 19 assertions; goes red on a one-character hand-edit to a generated file |
| `node tools/check-exits.mjs` | Every interior can be WALKED OUT OF, driven in the real engine with a real player holding a real direction. **Nothing proved a dungeon could be LEFT.** `walk-dungeons` floods ROOMS and the mouth room is reachable — it is where you arrive; `check-towns` asks the round-trip question of a town and of nothing else; `check-playthrough` walks D1 by a route that already knows the door is at x=4. Every one of them was green while a person stood inside Tidewash Grotto unable to find the way out: the mouth was one 16px `dStairs`, Link's hitbox is 10px, and of a 160px room exactly NINE PIXELS led outside (start x=60/64/68 left; 72/76/80 stopped dead against blank brick), while the room's own north opening is two tiles wide and puts you in the middle. Asserts, per interior — six dungeons, four caves, five houses — that doorway tiles and the `warps` list agree BOTH ways; that walking at the door from its own span AND one tile either side leaves, one tile either side being the reach of `Game.doorwayPull` rather than a fudge factor; that the pull is BOUNDED, so two tiles clear does NOT leave (without this the first claim would pass everywhere and mean nothing); and that the way out lands somewhere standable in a room that comes back. A door in a wall and a door standing in open floor get different claims, because only the first can be missed by alignment. 12 assertions go red with the pull disconnected |
| `node tools/check-sword.mjs` | The sword hits where Oracle of Seasons' sword hits. Its hit area is the cartridge's own (`SWORD_ARC`, `SWING_PHASE_FRAMES` in `feel.js`, read from the oracles-disasm disassembly at S149): live on EVERY frame of the swing, travelling from Link's side through the diagonal to full reach and back, against a 12x12 box on the middle of each ordinary enemy's sprite rather than its walking footprint. Parks Link in the real engine, stands a frozen octorok at 19 places around him, swings once and asserts hit or miss for each — including the misses the cartridge has (behind him, the side the swing ends on). The old box, straight out in front and live only mid-swing, missed 10 of the 12 places the original hits |
| `node tools/check-seasons-mechanics.mjs` | Seasons' three enemy mechanics, driven in the real engine (S152): a gel that touches Link clings (no sword, movement every other frame, button presses shake it off, then it hops away); a bubble's touch takes the sword for 180 frames; a spiked beetle cannot be hurt right way up, is flipped by Link's raised shield without costing him a hit, can be hurt on its back, and rights itself armoured again. And (S157) a lifted rock THROWN flies Seasons' own arc (`itemWeights` row 0: 1.5 px/f, 27 frames from carry height, about 40 px) both ways and hurts an enemy in its path for Seasons' 3 (`ITEM_BRACELET` damage `$fd`) — before S157 it broke at Link's feet the frame it was thrown. Asserted on the engine's own state. 18 assertions; the four throw assertions go red on the old throw |
...
| `node tools/check-side.mjs` | The side content can be WON (S155). Every prize the side content pays — the secret caves, the errands, the tide-pool race, the salvage dive — is taken in the real engine by the playthrough's own actor directives from a stated world, and most are also REFUSED without the thing they are meant to need (no bomb, wrong tide, no clock left). The robot never plays side content, so this is the only proof the moved heart pieces can still be had. `check-hearts` proves the count |
| `node tools/try-room.mjs '<json>'` | Nothing — it asserts nothing. Runs a few route directives in ONE room and prints where they left the player, so a route step can be tried in seconds instead of a full three-minute playthrough |
| `node tools/film-bosses.mjs [outDir] [every]` then `python3 tools/film-bosses.py <outDir>` | Nothing — it asserts nothing. Plays the real playthrough route and FILMS every boss and miniboss fight in it (a PNG every `every` frames while one is alive), then turns each into a real-speed GIF and a twelve-frame strip. Also prints how far each boss's drawn art ran past the top of the play area — with the camera scrolled, and with the camera as high as it goes — which a hitbox check cannot see because the art is taller than the hitbox. S153: every boss had been approved from stills |
| `node tools/shoot-region.mjs <x0> <y0> <x1> <y1> <out.png> [tide] [scale]` | Nothing — it asserts nothing. Stitches a rectangle of overworld screens into one picture (no HUD, no banners), the way the source games' map sheets show a town: how S157 put Tidewatch beside Horon Village for the human |
| `node tools/render-track.mjs <track> <out.wav> [loops]` | Nothing — it asserts nothing. Renders one of the game's own tracker tracks to a WAV through its own music engine, offline, so a person can LISTEN to it: a music change is judged by ear, and the human cannot run the game from a chat. S158 wrote it for the two town themes |
| `node tools/shoot-steps.mjs '<json>' <prefix>` | Nothing — it asserts nothing. `try-room`'s setup and directives, but it writes a 3x screenshot when each step named in `shots` finishes: how the side content was photographed for the human |
| `node tools/route-prefix.mjs <from> [<to>]` | Nothing — it asserts nothing. Plays the REAL route from a new game up to directive `<to>` in a few seconds (the run is ~90k frames to D4 and headless is fast), prints the trace from `<from>`, and with `HITS=<map>` every hit taken there. The real RNG history, which `try-room`'s stated setup cannot give; S140 re-routed all of D4 with it |
