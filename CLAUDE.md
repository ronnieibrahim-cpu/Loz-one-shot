...
| `node tools/check-camera.mjs` | The camera is pinned at 0,0 in every room no bigger than the view (264 of 273), never leaves the room, never moves more than `CAM_MAX_SPEED` in a frame, is always whole-pixel, ignores a player inside the deadzone, and actually follows in the 9 rooms that are wider than the view |
