...
- **A boss is not a picture on the cartridge (S152).** It is a list of 8x16
  ...
  oam flags`, with palette 6/7 loaded by the boss itself (`ld b,PALH_..`
  before enemyBoss_initializeRoom) — not by the room's tileset, which is
  where you look first and find nothing. `tools/rip-bosses.py` does it all;
  assemble through it, never by hand.
- **`Enemy.spriteName()` takes no game argument (S152).** A spec hook called
  from it that wanted `game` threw a ReferenceError at draw time — caught only
  because a scratch shot drew an enemy directly. Anything state-picked goes
  through `spec.pose(e)`.
- **Small hand-drawn boss art fails at procedural drawing (S152).** Eight
  rounds went into two 32px minibosses whose features were generated by
  shape code; what worked every time was a real Seasons part carrying the
  silhouette and texture, with drawn additions attached to it (never floating
  beside it), shown to the human large and in its room.
