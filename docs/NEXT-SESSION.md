## Two multi-phase bosses were never actually fighting fair — `Boss.phase` collided with `Entity.phase` (this session)

**The headline finding: every multi-phase boss in this game has been silently going unhittable and harmless partway through its OWN fight, for the whole life of the project, and it had nothing to do with any fighting verb.** Found by instrumenting `Boss.prototype.update` frame-by-frame (not guessed): `Boss`'s constructor set `this.phase = -1` and bumped it to the current fight-phase index (0, 1, 2, ...) on every phase change. `Entity`'s own constructor (`src/game/entity.js:90`) ALREADY owns a field called `.phase` — `opts.phase`, read by `Game.updatePhaseShift()` (`src/game/game.js`), meaning "the TIDE LEVEL this entity belongs to" (used for tide-variant enemies like a Leever that only exists at LOW). `Boss` shadowed that field with an unrelated number. The moment those two numbers diverged — the instant a boss's OWN fight-phase index stopped matching the room's ACTUAL tide level — `updatePhaseShift` read the boss as "phased out of this tide" and pinned `invuln = Math.max(invuln, 2)` on it **every single frame from then on**, plus `harmless = true`. Permanently unhittable, and it could no longer touch the player either.

Since every boss in this game fights at a FIXED fight tide (LOW=0, MID=1, HIGH=2 — see `FIGHTS` in `tools/check-bosses.mjs`) and starts its FIRST phase at index 0, a boss fought at LOW tide takes damage cleanly through its first phase (0 === 0) and then goes permanently unhittable the instant it changes phase (1, 2, ... never equal 0 again). A boss fought at HIGH (2) or MID (1) can be broken from the very first frame if its phase 0 doesn't happen to equal its fight tide. **This is not a difficulty problem or an "AI limitation" — every prior session's boss-verb notes calling Gohmaraq's cap "AI limitation" and Gloomtide's zero damage "the player is SWIMMING and a swimming Link cannot swing, fighting it means sinking with the Cleats first" were both describing the SYMPTOM of this one bug, not its cause.** Both diagnoses were reasonable given what was visible and both were wrong.

**Fix:** `src/game/enemy.js` — `Boss`'s own field renamed `this.phase` → `this.fightPhase` (constructor and the phase-transition block in `update()`). Nothing else in the codebase ever read `Boss.phase` (checked with a repo-wide grep before renaming) — `bosses.js`'s own phase callbacks always receive the index as a parameter, never read it off the entity. Boss instances now correctly read `phase === null` in `updatePhaseShift`, matching every other entity that doesn't care about tide-phase, and are skipped by it entirely — the correct behaviour, since a boss's OWN `weakOpen`/`shell` mechanic is what "is this thing hittable right now" should mean, not an accident of two unrelated numbers matching.

**Measured, before and after, in `check-bosses.mjs` (god mode — proves structure, not real-combat fairness):**

| Boss | Before this session | After |
|---|---|---|
| D1 Gohmaraq (LOW) | 10/24 hp, capped forever at phase 1 | **24/24 — full kill** |
| D2 Anemos (HIGH) | 0/30 hp | **30/30 — full kill** |
| D3 Gloomtide (MID) | 0/36 hp ("swimming blocks swinging" — wrong) | **36/36 — full kill** |
| D4 Wyverna (LOW) | 20/44 hp | 32/44 within the default budget; **44/44 confirmed with more frames** (see below) |
| D5 Rootmaw (LOW) | 20/52 hp | **52/52 — full kill** |
| D6 Nereth (MID) | 0/80 hp | **80/80 — full kill** |

Five of six bosses now die cleanly in god mode with the EXISTING generic `dBoss` verb — no boss-specific tactics, no new code beyond this bug fix. D4 needs a larger `['boss', N]` budget than the checker's default 9000 frames to finish (confirmed directly: it dies at frame 17574 with `['boss', 30000]`); it is not asserted by `check-bosses.mjs` either way since that checker deliberately does not assert a kill, but the annotation "(fight did not finish: AI limitation, see comment)" printed for D4 is now misleading — it isn't an AI limitation, it's a budget too short for a fight that is genuinely no longer capped.

**A second, related bug was found immediately behind the first, because fixing the first one let the actor reach it:** Gohmaraq in god mode, even with a 30000-frame budget, stalled forever at hp 6 with the ACTOR permanently pinned against a wall — `b.charging` was reading `true` forever. `charge()` (`src/game/enemy.js`) is the only thing that ever clears `e.charging`, and it does so by being called AGAIN on a later frame and finding the dash has hit a wall (`if (!moveDir(...)) { e.charging = false; ... }`). Gohmaraq's PHASE 2 never calls `charge()` at all (only phase 1 does) — so a boss whose hp crosses the phase-1→phase-2 threshold WHILE mid-dash carries `charging: true` into a phase that will never touch that field again, permanently. Every verb that reads `.charging` (`dBoss`'s dodge branch, added last session) then believes a charge is forever imminent and never does anything else. **Fix:** `Boss.update()`'s phase-transition block now sets `this.charging = false` unconditionally on every phase change — the one place that generically knows "we just moved to a phase that may not manage this state the same way," for any boss, not a Gohmaraq special case.

**A third, smaller bug in the ACTOR (not the engine) was found chasing the same stall down into real combat:** `dBoss`'s charge-dodge (`tools/actor-runtime.mjs`) picks which side to step to ONCE per charge, from which side of the boss's axis the player happens to be standing on — with no idea whether that side has a wall two tiles away. Measured directly in real combat (seed 20260806): a player near the arena's edge when a charge began picked the wall side, pressed into it doing nothing for the whole dash, and was run over. This is the exact "wall-cornering" failure mode a previous session's REVERTED ranged-dodge attempt hit and documented as unsolved with a one-time pick. The fix here is a LIVE check, not a smarter one-time pick (a fixed arena has no side that's "always open" — every side is a wall to a player already standing near it): if the last frame's press did not actually move the player, the side is blocked, so flip it. Comparing raw position frame-to-frame is a stronger signal than a recomputed cross-product sign (the thing that broke the reverted ranged-dodge) — it only ever flips after PROVING the current side does nothing.

**Real combat, measured (12 quarter-hearts, no god mode, seed 20260806, LOW tide): still not a win, but the picture changed.** Before this session's phase/charging fixes: 5 hits landed, 24 → 14 hp, dies. After: **7 hits landed, 24 → 10 hp**, dies to a fresh contact hit during a SECOND charge — one the liveness-flip successfully dodged earlier in the same fight, but this one happened in a genuine CORNER (both perpendicular directions blocked), where a single flip isn't enough. This is the SAME class of problem the reverted ranged-dodge could never solve either ("the arena has walls, and the latched side can dead-end into one mid-dodge, pinning the player against the fence with only one axis of the dodge still live"). The periodic slam-triggered shot burst (last session's finding) is STILL present and unaddressed; the corner-trapped charge is simply what kills the player first now.

**`tools/check-bosses.mjs` also needed a real fix, unrelated to the game itself:** its per-fight sampling loop pumped 400 frames at a time before taking one snapshot of `weakOpen`. That used to be far shorter than any real fight (every boss was capped, so fights ran the whole 9000-frame budget). Now that fights can finish in ~300 frames (Gloomtide), a SINGLE 400-frame chunk can contain the entire fight, so the one snapshot taken afterward reads a dead, cleared boss and reports "eye never opened" — true of the sample, false of the fight (this is exactly what happened to D3 on the first post-fix run: `boss: gone` before a single sample landed). Fixed by shrinking the chunk to 40 frames (guard cap raised to 40000 to keep the same total budget) — short enough that even the fastest fight measured takes several chunks.

**A second, unrelated but very expensive rabbit hole this session: `tools/walk-dungeons.mjs`'s ledge-hop verification (part 3) was measuring real WALL-CLOCK time, not game frames, and it had a hop with essentially zero margin.** After the fixes above, `walk-dungeons.mjs` started failing one previously-green ledge hop (`overworld 0,0,0 '_' down @3,5`) — on a room with no boss in it at all, which made no sense at first. Root cause, found by bisection down to a single line and then proven with a control experiment: the file's OWN intro-cutscene-skip loop (`await page.keyboard.press('Enter'); await frames(6);` etc.) measured "wait until `g.frame` has advanced by N" against `requestAnimationFrame` — i.e., against however much REAL TIME the browser actually took to deliver N callbacks, not against a controlled, deterministic step count. That loop's own exit condition (`mode === 'cutscene'`) means a ONE-FRAME difference in when a keypress registers can cost or save a WHOLE extra loop iteration, leaving the player object with different residual state (almost certainly `frozen`, from the intro's own conch-grant animation — the tracked player sat dead still for 20 of the ledge test's 22 held-key frames before finally moving) for the rest of that page's entire life, including a ledge test running MANY rooms later. Proved with a control: padding an UNRELATED file with two comment lines — zero functional change — was enough to shift this boot sequence's real-time/game-frame ratio and flip the SAME check from pass to fail. This is not "intermittent" in the flaky sense (it is 100% reproducible for any FIXED code state) but it is unrelated to whatever code actually changed, which makes it worse: it would have blamed the next completely unrelated diff too.

**Fixed properly rather than worked around:** `walk-dungeons.mjs` now calls `window.__harness.takeOver()` (the same stepping hook `tools/replay.mjs`/`tools/actor-runtime.mjs` already use, `src/main.js:79`) once, right after the page loads, and replaces every real-time wait — the intro-skip loop AND the whole ledge section — with `window.__harness.step(n)`, which calls `game.update()` exactly n times with no real-clock involvement at all. `page.keyboard.down`/`up` still drive which keys are HELD (real DOM key state, unaffected by timing); only the frame-advancement stopped being at the mercy of the clock. Verified deterministic: 3 repeat runs, byte-identical results, both before this fix existed (reproducing the failure) and after (41/41 clean) — and a direct A/B against the un-fixed methodology proved the earlier "it fails" and "it passes" runs were BOTH real, just riding whichever side of a razor-thin timing margin the browser happened to land on that run.

**Also fixed while in the neighbourhood (a "good first job" `docs/HANDOFF.md` had been carrying for two sessions):** `check-motion.mjs`, `check-charms.mjs`, `check-items.mjs`, `check-trade.mjs`, `find-ledges.mjs` and `preview.mjs` were the last tools in `tools/` missing the `CHROMIUM_PATH` fallback `test.mjs`/`check-build.mjs`/`check-bosses.mjs` already carry for this container's off-by-one-revision Playwright install. All six now have it, verified individually green.

**One replay had to be re-recorded, and this is the expected shape of a real boss-behaviour fix, not a regression** (CLAUDE.md's own hard-won lesson: "a five-line change to the movement path is never a five-line change"): `tools/replays/d1-clawcrab-den-wide.json` fights the Clawcrab, a `defineBoss` miniboss, and its recorded tape diverged by exactly one quarter-heart once the boss stopped being artificially frozen partway through. Re-recorded with `node tools/replay.mjs --record d1-clawcrab-den-wide`; `replay.mjs` is 51/51 again.

**Full regression, run individually, all green:** `test.mjs` 59/59, `check-bosses.mjs` 13/13, `replay.mjs` 51/51, `check-playthrough.mjs` 19/19, `walk-dungeons.mjs` 23/23, `solve-switches.mjs` 9/9, `check-gates.mjs` 26/26, `check-motion.mjs` 8/8, `validate.mjs`, `check-overworld.mjs` 17/17, `check-progression.mjs` 19/19, `check-anchor.mjs` 14/14, `check-cleats.mjs` 15/15, `check-lens.mjs` 24/24, `check-bellows.mjs` 60/60, `check-reefseed.mjs` 87/87, `check-dredge.mjs` 103/103, `check-towns.mjs` 58/58, `check-hearts.mjs` 114/114, `check-music.mjs` OK, `check-torches.mjs` 5/5, `check-charms.mjs` 63/63, `check-items.mjs` 91/91, `check-trade.mjs` 43/43, `check-build.mjs` OK, `npm run build` clean.

**How all of this was measured:** the same scratch harness pattern as last session (not committed, matching every prior "real combat" session's own precedent) — `beginRecord` with `godMode:false, hearts:12, maxHearts:12`, patching `Boss.prototype.hurt`/`update` and `Player.prototype.takeDamage`/`startSwing`/`updateSwing` on the live CLASS before `beginRecord` boots the game, so every instance created by entering the room is already instrumented. The godmode structural numbers came from small, temporary edits to a LOCAL COPY of `check-bosses.mjs`'s own fixture (larger `['boss', N]` budgets, per-fight isolation) — never committed, the shipped `check-bosses.mjs` only carries the CHUNK-size fix.

**Next session, in order:**

1. **The corner-trapped charge is the one thing left standing between "narrowed" and "won."** The liveness-flip (this session) fixes a charge that starts near ONE wall; it does not fix a genuine corner where BOTH perpendicular directions are blocked. Two ideas worth trying, neither attempted here: (a) when the flip itself proves stuck too (two blocked frames in a row, not one), step diagonally away from the corner instead of purely perpendicular — untested, and diagonal escape from a charge's line has its own geometry to get right; (b) read the charge's OWN tell window (`e.stun` during `windUp`, which freezes the boss and gives 14-22 warning frames depending on phase) to reposition to open ground BEFORE the dash starts, rather than reacting once it's already moving. (b) is likely more robust — the boss is frozen the whole time, so there is no rush and no risk of dodging into the dash's own path — but has not been measured.
2. The periodic slam-triggered shot burst (found and left open two sessions running now) is still the OTHER standing problem, independent of the charge. `docs/NEXT-SESSION.md`'s own archived section below has the geometry and the reason a reactive per-shot dodge failed twice; a fix that reads the slam's timer directly (predictable, not reactive) rather than the shot's position after the fact was the untried idea left on the table.
3. Once Gohmaraq is a measured win at 3 hearts, wire `dBoss` into `playthrough-route.mjs` past `d1/0,3,2`, and only then look at the other five bosses — now that god mode confirms 5 of 6 die cleanly with the generic verb, Gloomtide's "needs the Cleats" diagnosis in `check-bosses.mjs`'s own header comment is WRONG and should be corrected or re-verified in real combat before the next session trusts it.
4. The Boss Key / third-key pass behind the Clawcrab door and the other five dungeons' routes are both still undone and both still blocked on job 1 actually finishing.

---

## Contact damage is gone; the shot burst is the whole problem now — the boss verb, continued (previous session)
