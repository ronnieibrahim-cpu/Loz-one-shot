...
| D4's Ironknight Gallery (miniboss room) widened from `2x1` to the game's second `3x1`/`2x2`-class room, under the standing session charter's "1 wide-rooms" objective — 2 of 6 dungeons now qualify (D4, D6), still short of the 3-of-6 bar. Survey script bug found and fixed before trusting it: a naive "is this key occupied" check misses the second cell of an existing `2x1` room | S68 (charter S3) | `docs/NEXT-SESSION.md` S68; `docs/DUNGEON-STATUS.md` D4 section |
| D5's Thornvine (miniboss room) widened from `1x1` to `3x1`, closing the standing session charter's "1 wide-rooms" objective at 3 of 6 (D4, D5, D6). Kelp Locks (D3) and Shrine Ford (D5) both had a free growth cell too but were passed over for carrying custom room-geometry objects (`cleatRoom`, `reefseedRoom`); Thornvine, like Ironknight Gallery before it, has neither. Rotation advances to #2, art-provenance | S69 (charter S4) | `docs/NEXT-SESSION.md` S69; `docs/DUNGEON-STATUS.md` D5 section |
| `tools/shoot-sprites.mjs` built — one real-pixel, labelled contact sheet per sprite pack, at 1x and 3x, covering every sprite in `src/data/sprite-manifest.js` (327 manifest entries, 30 sheets, 0 missing in any pack). The art-provenance objective's other done-condition, tagging all 561 `sprites-*.js` entries, is still open; this only builds the tool to verify tags against once tagging starts | S70 (charter S5) | `docs/NEXT-SESSION.md` S70 |
