...
| **CORRECTION — the player's guide lives on `main`, not on a branch.** `docs/GUIDE.md` (2,083 lines), `docs/GUIDE.html` (3,048 lines) and `tools/check-guide.mjs` (234 lines) are all committed on `main`, last touched 2026-09-04 (commit `b6a8e7f`, "Link swings the sword instead of the sword appearing"). Three open branches carry an OLDER, SMALLER guide and predate that commit: `claude/guide-walkthrough-revision-jgs0uf` (2026-08-21, `docs/GUIDE.md` 2,076 lines), `claude/oracle-tides-guide-7ak0mw` (2026-08-21, 738 lines), `claude/player-walkthrough-guide-74mtr5` (2026-08-20, 680 lines, the branch that first added `docs/GUIDE.md`/`check-guide.mjs`). **Do not merge any of them** — every one would overwrite `main`'s current, larger guide with a stale one. Anyone reading a note that says the guide lives on a branch is reading something out of date | S68 | this row (verified directly against `main` and all three branches via `git log`/`git show`, 2026-09-11) |
