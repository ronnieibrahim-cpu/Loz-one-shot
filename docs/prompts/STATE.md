...
S18 | objective | gave darknut a hurtFrame (4th, after wisp/beetle/wisp again) — checked the sheet FIRST this time: cropped and viewed every box in the Darknut plate (idle x2, side x2, walk-leg pairs, an overhead swing windup, four side-lunge poses, three weapon icons), all aggressive stances, no flinch, so hand-drew darknut_hurt (sprites-enemies-hurt.js) reusing darknut_d0's grid pixel-for-pixel — only the two visor eye-dots and the nose bridge between them turned black (eyes shut, boss roster's own grammar), 3 pixels, silhouette untouched. Eligible under S12's hp rule (6 > swordDamage() 2). Verified in-engine (scratch Playwright probe, not committed): non-lethal hit showed darknut_hurt for the full 24-frame flicker window (hp 6->5), screenshotted mid-flicker showing the shut visor under the invuln tint, then reverted to the walk cycle on schedule. check-drift now reads darknut: walk,hurt. check-rippers 17/17 (sprites-enemies.js untouched, confirmed byte-identical before starting). validate/test(83/83)/check-build all green, dist rebuilt
