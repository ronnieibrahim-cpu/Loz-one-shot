...
DETOUR TOKENS: 0 (spent S95; needs 2 consecutive objective sessions to regen)

SESSION LOG: one row per session — `S## | objective|detour | one line`
S95 | detour | Fixed check-bellows.mjs's overworld filter bug (index derived as 0 for any overworld room, always read as "before the Bellows" — same shape as S91's check-anchor.mjs fix); 78/78 unchanged on existing D4/D5/D6 rooms. Then found the REAL blocker the filter fix doesn't touch: every dungeon wheel is boxed by dPit, the one tile impassable in every mode at every tide yet not F.SOLID (so the cone still crosses it) — and no outdoor tile has that combination (confirmed by grep: solid rock blocks the cone too; chasm is hoppable; deep water is swimmable; drownWall turns swimmable exactly when a HIGH wheel needs it blocked). Bellows' overworld half stays at 0 of 3 needed; needs a new tile or a different outdoor model, not a filter change. No game room touched — argument verified by tile-flag grep, not a built test case. Full regression green. Full writeup: docs/prompts/LEDGER.md "Known and deliberately unfixed", docs/NEXT-SESSION.md S95.
