...
S74 | objective | Gave The Gyre and Drowned Hollow their own careful pass, as S73 deferred. Both confirmed clean and unrelated to the historical tree-crown severing: The Gyre's closed riptide ring and mud/ledge centre match the room's own comment exactly (swim it and the current carries you round; sink and you walk straight through — the Cleats' whole argument), and `check-strands.mjs`'s baseline 10-cell region there is that centre, unreached by the checker's on-foot-only flood, not a severed corridor. Drowned Hollow's three `waterD` bands are fixed-deep at every tide by design; its baseline entries are single border cells, matching the already-documented root-pocket pattern. Re-ran `check-strands.mjs` after looking at both and it's unchanged from baseline. Zero new defects, zero src/ changes. check-drift's audit count now 60 of 120. **The Drowned Wood is fully audited: 15/15.** Full regression: check-drift self-checks OK, check-playthrough 21/21, check-strands OK (baseline), npm run build (dist unchanged)
