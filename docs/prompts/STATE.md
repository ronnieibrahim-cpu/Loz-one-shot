...
S73 | objective | Per NEXT-PROMPT.md's own task, checked the Drowned Wood's row 3 minus The Gyre (Wood Edge, Rotting Grove, Wood Gate) and row 4 minus Drowned Hollow (Shrine Path, Shrine Mouth, Log Drift) — both flagged rooms left for their own dedicated session per CLAUDE.md's hard-won-lessons list. One more spawn-position false alarm caught and cleared the same way as S70's Dune Corner: Shrine Path's default screenshot spot puts Link almost on top of zol's own spawn tile, so zol's tall sprite pokes up over Link's head and briefly reads as odd-coloured hair; re-shot a few pixels south and zol is its ordinary clean sprite on its own. Shrine Mouth's D5 dungeon gate (stone arch, carved emblem) confirmed as a distinct, proper entrance, not reused/placeholder art. Zero new defects, zero src/ changes. check-drift's audit count now 58 of 120, split 15 Tidewatch Village / 12 Marsh / 18 Dunes / 13 Wood. Full regression: check-drift self-checks OK, check-playthrough 21/21, check-strands OK (baseline), npm run build (dist unchanged)
