...
FILE ALLOWLIST for the current objective (4 enemy-roster):
  ...
  src/data/sprites-enemies-hurt.js (hand-drawn hurtFrame/deathFrame art the
    sheet lacks; S12 wisp_hurt, S13 stalfos_death, S15 beetle_hurt — new
    keys in an EXISTING entry need only a sprite-manifest.js edit)
...
S15 | objective | gave beetle its first real hurtFrame, the third proof of the S12 mechanism: sheet has no recoil pose for the Spiked Beetle (only its 2 upright + 2 balled-charge frames), so hand-drew beetle_hurt (sprites-enemies-hurt.js) — beetle_d0's outline/antenna/legs pixel-for-pixel unchanged, a single 7-pixel zigzag crack punched down the shell's dead centre, fitting since beetle has shield:'front' and this only ever shows on a hit that got past that shield. Wired hurtFrame + sprite-manifest.js entry; hp 3 > swordDamage() 2, so it's eligible under the S12 rule. Verified in-engine: hit for 1 (non-lethal, hp->2), beetle_hurt held for the whole 24-frame flicker window, then returned to the walk cycle; screenshotted, crack visible in enemyk's bone tones. check-drift now reads beetle: walk,hurt. validate/test(83/83)/check-motion/check-build all green, dist rebuilt
