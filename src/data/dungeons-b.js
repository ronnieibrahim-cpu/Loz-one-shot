...
export function installDungeonsB() {
  ...
  registerMap({
    ...
    rooms: {
      ...
      '0,5,3': {
        name: 'Thornvine',
        // The miniboss, at room 20 of 24. Three screens wide, the same reason
        // D4's Ironknight Gallery grew: one lagoon this size gives the fight
        // room a single 10-tile basin never had. `(6,3)` and `(7,3)` had
        // nothing in them — no room, no warp — so the old east wall opened
        // into more of the same basin rather than a second fixture. The
        // floor is `2` (`dBasin`: dry at LOW, damp at MID, shallow at HIGH)
        // the whole width, so nothing here needs an item to cross at any
        // tide — widening it changes how much room the fight has, not what
        // it takes to reach.
        size: [3, 1],
        map: [
          '####.#########################',
