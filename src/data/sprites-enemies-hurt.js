...
export const ENEMY_HURT_ART = {
  ...
  // Tektite death — drawn: the sheet's own "Tektite" plate has exactly two
  // frames (legs splayed on the ground, legs tucked mid-hop), both already
  // extracted as `tektite_0`/`tektite_1` — confirmed against the sheet's own
  // "Tektite" label spanning only those two boxes, "Stalfos" and "Thwomp"
  // sitting either side of it as unrelated creatures. Nothing to extract;
  // hand-drawn instead, following `octorok_death`'s own squash rather than
  // inventing a new shape: every other row of `tektite_0`'s body (the
  // splayed-legs pose, rows 5/7/9/11/13/15 of its 16-row grid in
  // sprites-enemies.js — the resting pose, not `tektite_1`'s hop-apex tuck)
  // kept pixel-for-pixel and compressed to the bottom of the cell, reusing
  // only colour indices already present in the two live frames so it renders
  // correctly under `tektite`'s own palette with nothing invented. Reads as
  // the eight-legged body pressed flat rather than a generic squash, the
  // same "flatten the existing shape into a puddle" move `octorok_death`/
  // `gel_death` used. `tektite` (hp 2, `src/data/enemies.js`) has no `z`
  // field — confirmed directly rather than assumed — so a low pose is
  // correct; it does drive `hop()` (`src/game/enemy.js`), whose airborne
  // `fz` is a per-frame accumulator distinct from a spec `z` field, and
  // `Enemy.update()`'s `dying` branch skips `ai()` entirely, so a kill
  // mid-hop freezes `fz` at whatever height it held and the flat pose draws
  // there rather than snapping to the ground — verified in-engine, not
  // assumed, and noted rather than treated as a defect (see this session's
  // NEXT-SESSION.md entry). No hp constraint applies to `deathFrame`; hp 2
  // means `swordDamage()` always kills `tektite` in one hit, so this is its
  // only possible death/hit feedback, same as `octorok`/`crab`.
  tektite_death: `
    ................
    ................
    ................
  ...
  // Beetle death — drawn: the sheet's own "Spiked Beetle" plate has exactly
  // four frames (upright front x2, balled-up side x2), all already
  // extracted as `beetle_d0`/`d1`/`s0`/`s1` — confirmed against the sheet's
  // own "Spiked Beetle" label spanning only those four boxes, "Spinning
  // Blade" sitting to the right as an unrelated creature. Nothing to
  // extract; hand-drawn instead. Squashed the UPRIGHT pose (`beetle_d0`,
  // the resting/walking shape) rather than uncurling the balled-up
  // `charge()` shape (`beetle_s0`/`s1`) — a squash of the standing form
  // reads as "collapsed," where uncurling the charge ball could as easily
  // read as "about to charge," the wrong direction for a death pose.
  // Following `octorok_death`'s own every-other-row squash rather than
  // inventing a new shape: rows 0/2/4/6/8/10/12/14 of `beetle_d0`'s 16-row
  ...
  // in the live frames. `beetle` (hp 3, `src/data/enemies.js`) has no `z`
  // field, confirmed directly. `beetle` also has `shield: 'front'` — the
  // first `deathFrame` target with a shield checked against a LETHAL hit
  // by this thread (`crab`, S29, has the same field but the interaction
  // was never explicitly probed): verified in-engine that a hit arriving
  // at the shielded front is blocked outright regardless of damage
  // (`hurt()` returns `false`, `hp`/`dying` untouched, even for a
  // would-be-lethal amount), and that a hit from an unshielded angle goes
  // through normally — a non-lethal one (hp 3 -> 1) shows `beetle_hurt`
  // and reverts to the walk cycle, the lethal follow-up shows
  // `beetle_death` and holds for the full stall.
  beetle_death: `
    ................
    ................
    ................
  ...
  // Bubble death — drawn: no snuffed-out frame on the plate. The skull alone
  // with its ring of sparks gone and its eyes shut, centred in the cell (it
  // flies, so it fizzles where it hangs, the way wisp_death does), with
  // four last sparks flying off the corners. The bubble cannot die in this
  // game; the pose is ready for the day something is allowed to kill it.
  bubble_death: `
    ................
    ..2..........2..
    ................
  ...
  // Beamos death — drawn: the sheet has no broken statue. beamos_1 with its
  // eye gone dark and a crack running from the cap down through the socket.
  // Unkillable here, like the bubble: ready, not reachable.
  beamos_death: `
    ................
    ................
    ......3333......
  ...
  // Barnacle death — drawn: the Like Like plate is five gape frames and no
  // collapse. barnacle_atk's squat shell sunk lower still, its mouth closed
  // to one dark line — a husk. Unkillable here too: ready, not reachable.
  barnacle_death: `
    ................
    ................
    ................
