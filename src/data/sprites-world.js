// Sprite pack: pickups, world objects, projectiles, and NPCs.
//
// Art grammar: see src/gfx/art.js ('.' transparent, '0'..'3' light to dark).
// Required names and sizes: see src/data/sprite-manifest.js.
//
// NPC_ART only carries the two characters original to this game. The other
// nine NPCs are extracted from the Oracle of Seasons sheet in sprites-npcs.js,
// which installs after this pack and wins — see docs/ART-DIRECTION.md rule 1,
// "if a sheet has it, extract it".

import { sprites } from '../gfx/art.js';

// 16x16 pickups.
export const PICKUP_ART = {

  // The healing fairy is EXTRACTED — see src/data/sprites-fairies.js and
  // tools/rip-fairies.py. It was hand-drawn here, and it was the one sprite in
  // this file that actually broke the outline rule (eight body pixels on the
  // wings' inner edges touched bare air). The sheet had it all along.

  // The rupees, hearts, keys, Piece of Heart and Heart Container that were
  // drawn here are the cartridge's own now: src/data/sprites-treasures.js,
  // from tools/rip-treasures.py (S155).

  // ---- the Essences of the Tide: one per dungeon, and SIX DIFFERENT BELLS.
  //
  // There used to be one orb, drawn twice, shown on all six title cards and
  // six times over in the quest menu. The source draws a different icon per
  // essence and a player notices by about the fifth one; ours are also called
  // six different things — the Shallow Bell, the Coral Bell, the Bog Bell, the
  // Cliff Bell, the Drowned Bell, the Drowned King's Bell — so one shape for
  // all of them was the art contradicting the text.
  //
  // The FAMILY is the bell: same silhouette, same loop, same rim, same
  // clapper, so six of them in a row read as one set. What varies is the mark
  // on the body, in the palette's mid blue against the white: it is the only
  // part of each icon that is different, which is what makes the difference
  // legible at 16 pixels rather than six shapes that are all a bit vague.
  //
  // `_1` is the twinkle frame: four sparks off the shoulders and the flare,
  // clear of the bell's own outline. `_dim` is the menu's unlit slot, the same
  // shape with every body tone hand-set to the shade rather than computed by a
  // palette swap, because a palette swap would take the outline with it.
  // THE TIDE BELL ITSELF, unmarked: the six shards put back together. The
  // ending card and the generic essence card both hold this one up, and the
  // whole point of it is that it has no mark on it — six marked bells become
  // one plain one.
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_tidebell_0: `
    ................
    .......33.......
    ......3003......
    ......3003......
    .....300003.....
    ....30000003....
    ....30000113....
    ...3000001113...
    ...3000011113...
    ..300000011113..
    ..300001111113..
    .30000001111113.
    .33333333333333.
    .....300003.....
    ......3113......
    .......33.......`,

  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_tidebell_1: `
    ................
    .......33.......
    ..0...3003...0..
    ......3003......
    .....300003.....
    ....30000003....
    ....30000113....
    .0.3000001113.0.
    ...3000011113...
    ..300000011113..
    ..300001111113..
    .30000001111113.
    .33333333333333.
    .....300003.....
    ......3113......
    .......33.......`,
  // Essence 1, the Shallow Bell — two tide lines across the body; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence1_0: `
    ................
    .......33.......
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    ...3022222223...
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 1, the Shallow Bell — two tide lines across the body; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence1_1: `
    ................
    .......33.......
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    .0.3022222223.0.
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 1, the Shallow Bell — two tide lines across the body; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence1_dim: `
    ................
    .......22.......
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    .22222222222222.
    .22222222222222.
    .......22.......
    .......22.......`,

  // Essence 2, the Coral Bell — a coral sprig; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence2_0: `
    .....3....3.....
    ......3333......
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    ...3022222223...
    ..302222222223..
    .30311133111313.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 2, the Coral Bell — a coral sprig; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence2_1: `
    .....3....3.....
    ......3333......
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    .0.3022222223.0.
    ..302222222223..
    .30311133111313.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 2, the Coral Bell — a coral sprig; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence2_dim: `
    .....2....2.....
    ......2222......
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    .22222222222222.
    .22222222222222.
    .......22.......
    .......22.......`,

  // Essence 3, the Bog Bell — a bog reed; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence3_0: `
    ................
    .......33.......
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    ...3022222223...
    ..302222222223..
    .30111111111113.
    .33333333333333.
    ...3...33...3...
    .......33.......`,

  // Essence 3, the Bog Bell — a bog reed; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence3_1: `
    ................
    .......33.......
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    .0.3022222223.0.
    ..302222222223..
    .30111111111113.
    .33333333333333.
    ...3...33...3...
    .......33.......`,

  // Essence 3, the Bog Bell — a bog reed; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence3_dim: `
    ................
    .......22.......
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    .22222222222222.
    .22222222222222.
    ...2...22...2...
    .......22.......`,

  // Essence 4, the Cliff Bell — a terrace of steps; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence4_0: `
    .......33.......
    ......3333......
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    ....30222223....
    ...3022222223...
    ..301111111113..
    ..333333333333..
    ....3..33..3....
    .......33.......`,

  // Essence 4, the Cliff Bell — a terrace of steps; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence4_1: `
    .......33.......
    ......3333......
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .0..30222223..0.
    ...3022222223...
    ..301111111113..
    ..333333333333..
    ....3..33..3....
    .......33.......`,

  // Essence 4, the Cliff Bell — a terrace of steps; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence4_dim: `
    .......22.......
    ......2222......
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    ..222222222222..
    ....2..22..2....
    .......22.......`,

  // Essence 5, the Drowned Bell — a ribbon of kelp; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence5_0: `
    ................
    .......33.......
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    .....301113.....
    .....301313.....
    .....301313.....
    ....30113113....
    ...3022232223...
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 5, the Drowned Bell — a ribbon of kelp; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence5_1: `
    ................
    .......33.......
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    .....301113.....
    .....301313.....
    .....301313.....
    ....30113113....
    .0.3022232223.0.
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 5, the Drowned Bell — a ribbon of kelp; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence5_dim: `
    ................
    .......22.......
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    .22222222222222.
    .22222222222222.
    .......22.......
    .......22.......`,

  // Essence 6, the Drowned King's Bell — a crown; lit
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence6_0: `
    ....3..33..3....
    ....3.3333.3....
    .......33.......
    ......3333......
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    ...3022222223...
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 6, the Drowned King's Bell — a crown; twinkle
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence6_1: `
    ....3..33..3....
    ....3.3333.3....
    .......33.......
    .0....3333....0.
    ......3013......
    ......3013......
    .....301113.....
    .....301113.....
    .....301113.....
    ....30111113....
    .0.3022222223.0.
    ..302222222223..
    .30111111111113.
    .33333333333333.
    .......33.......
    .......33.......`,

  // Essence 6, the Drowned King's Bell — a crown; unlit, for the quest menu
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  p_essence6_dim: `
    ....2..22..2....
    ....2.2222.2....
    .......22.......
    ......2222......
    ......2222......
    ......2222......
    .....222222.....
    .....222222.....
    .....222222.....
    ....22222222....
    ...2222222222...
    ..222222222222..
    .22222222222222.
    .22222222222222.
    .......22.......
    .......22.......`,
};

// 16x16 world objects.
export const OBJECT_ART = {
  // Chests, the sign, push blocks, floor buttons and torches are Seasons' own
  // since S156: tools/rip-objects.py -> sprites-objects.js.

  // ---- raft: LEFT HALF only, the engine mirrors it to 32 wide ------------
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  o_raft: `
    3333333333333333
    3011111111111111
    3011111111111111
    3333333333333333
    3011111111111111
    3011111111111111
    3333333333333333
    3011111111111111
    3011111111111111
    3333333333333333
    3011111111111111
    3011111111111111
    3333333333333333
    3011111111111111
    3011111111111111
    3333333333333333`,

  // ---- the sluice valve: shut, then spun open ----------------------------
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  o_valve: `
    ................
    ......3333......
    ....33111133....
    ...3111111113...
    ..311111111113..
    .31111333311113.
    .31113....31113.
    .31113333331113.
    .31113333331113.
    .31113....31113.
    .31111333311113.
    ..311111111113..
    ...3111111113...
    ....33111133....
    ......3333......
    ................`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  o_valve_open: `
    ................
    ......3333......
    ....33111133....
    ...3111111113...
    ..311133311113..
    .31113331111113.
    .31111333111113.
    .31111133311113.
    .31111133311113.
    .31111333111113.
    .31113331111113.
    ..311133311113..
    ...3111111113...
    ....33111133....
    ......3333......
    ................`,

  // The same wheel a quarter-turn on: o_valve turned about its centre, so
  // the slot stands upright. Alternating the two while wind is on it is what
  // makes a wheel that is TURNING look different from one that is not.
  // derived — o_valve rotated 90 degrees, no new pixels.
  o_valve_turn: `
    ................
    .....333333.....
    ....31111113....
    ...3111111113...
    ..311111111113..
    ..311133331113..
    .31113.33.31113.
    .31113.33.31113.
    .31113.33.31113.
    .31113.33.31113.
    ..311133331113..
    ..311111111113..
    ...3111111113...
    ....31111113....
    .....333333.....
    ................`,

  // ---- clay pot and a liftable boulder -----------------------------------
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  o_pot: `
    ................
    .....333333.....
    ....31111113....
    ...3111111113...
    ..311110111123..
    ..311111111223..
    ..311111112223..
    ..311111122223..
    ..311111222223..
    ..311112222223..
    ..331122222233..
    ...3312222333...
    ....33333333....
    ................
    ................
    ................`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  rock16: `
    ................
    ................
    .....333333.....
    ....31111113....
    ...3111111113...
    ..311111111113..
    .31100000000113.
    .31022000000013.
    .31000022000013.
    .31100000220113.
    ..311111111113..
    ...3111111113...
    ....31111113....
    .....333333.....
    ................
    ................`,
};

// 8x8 projectiles. `shot_spear` points RIGHT; the engine mirrors it.
export const SHOT_ART = {
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot: `
    ..3333..
    .300003.
    30000003
    30000003
    30000003
    30000003
    .300003.
    ..3333..`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_rock: `
    ..3333..
    .311113.
    31122113
    31222213
    31222213
    31122113
    .311113.
    ..3333..`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_bubble: `
    ..3333..
    .300003.
    30100003
    30000003
    30000003
    30000003
    .300003.
    ..3333..`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_beam: `
    ...33...
    ..3003..
    .300003.
    30000003
    30000003
    .300003.
    ..3003..
    ...33...`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_spear: `
    ........
    ......33
    .....300
    33333000
    33333000
    .....300
    ......33
    ........`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_orb: `
    ..3333..
    .311113.
    31100113
    31000013
    31000013
    31100113
    .311113.
    ..3333..`,
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  shot_ink: `
    ..3333..
    .311113.
    33111133
    31111113
    31111113
    33111133
    .311113.
    ..3333..`,
};

// 16x16 NPCs, drawn facing the viewer. Only the two original to this game —
// the rest are extracted in sprites-npcs.js, which installs after this pack.
export const NPC_ART = {
  // The Maku Tree: a broad canopy with a face in the bark, on a stubby trunk.
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  npc_maku: `
    ....33333333....
    ..331111111133..
    .33111111111133.
    3311111111111133
    3110033003300113
    3110033003300113
    3111111111111113
    3111003333001113
    .31110000001113.
    .33111111111133.
    ..331111111133..
    ....33111133....
    ......3113......
    ......3113......
    .....331133.....
    ....33333333....`,

  // Nereth, the Drowned King: a crowned skull over a robe of falling water.
  // drawn — original to this game, no source sheet has anything like it (assets/sheets/README.md).
  npc_nereth: `
    ...3..3..3..3...
    ...3333333333...
    ...3111111113...
    ....33333333....
    ....30000003....
    ....30300303....
    ....30000003....
    ....30333303....
    ....33333333....
    ...3111111113...
    ..311111111113..
    .31111111111113.
    .31122222222113.
    .31222222222213.
    ..322222222223..
    ...3333333333...`,
};

export function installWorldSprites() {
  sprites.add(PICKUP_ART, 'rupee');
  sprites.add(OBJECT_ART, 'wood');
  sprites.add(SHOT_ART, 'enemyr');
  sprites.add(NPC_ART, 'npc');
}
