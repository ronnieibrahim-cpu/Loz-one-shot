...
export class Boss extends Enemy {
  constructor(x, y, spec, opts = {}) {
    ...
    // NOT `this.phase` — `Entity.phase` (set in `Entity`'s own constructor,
    // always `null` unless an entity is spawned with a `phase:` option) is a
    // completely different thing: which TIDE LEVEL a D2-style phasing enemy
    // belongs to. `Game.updatePhaseShift` reads `e.phase` on every entity to
    // decide whether it is "not in this tide" and should go hidden, harmless,
    // and permanently re-armed at `invuln >= 2` — and every Boss used to set
    // `this.phase` to its own 0/1/2 combat-phase index, a number of exactly
    // the same shape. The two happened to agree during a boss's very first
    // combat phase whenever that boss's own fight tide is LOW (index 0 ==
    // tide 0), which is why this went unnoticed: Gohmaraq (LOW) landed its
    // first five hits fine and then went permanently untouchable — hidden,
    // harmless, re-invulnerable every single frame — the instant it left
    // phase 0, because index 1 no longer equalled tide 0. `combatPhase` is
    // the same counter, under a name nothing else in the engine reads.
    this.combatPhase = -1;
