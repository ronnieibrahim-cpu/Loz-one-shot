...
// feather gates, only the bombable one. `--bombs` makes F.BOMBABLE passable and
// is what proves the Marsh gate.
//
// Usage: node overworld.mjs [--bombs]
...
installData();
const BOMBS = process.argv.includes('--bombs');
...
function walkableAt(legend, ch, tide) {
  ...
  if (!d) return false;
  if (BOMBS && (d.flags & F.BOMBABLE)) return true;
...
console.log(`  flood${BOMBS ? ' (with bombs)' : ''}: ${reached.size}/120 screens, ${seen.size} tiles`);
if (BOMBS) check('with Bombs, all 120 screens are reachable', unreached.length === 0, unreached.join(','));
else {
  // Without Bombs the Marsh is sealed: that is the gate working, not a bug.
  check('without Bombs the Marsh is sealed', unreached.length > 0, 'nothing was gated');
  console.log('  gated without Bombs: ' + unreached.join(','));
}
