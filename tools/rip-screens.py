...
WHAT IS OURS IN THE OUTPUT, AND WHAT IS NOT. Every pixel here is the source's
except four things, each done by rule rather than by hand:
  ...
  4. The logo's red is Farore's green: Seasons is red, Ages is blue, Tides is
     green. One colour replaced by one colour, nothing else touched.
...
GREEN = tuple(round(v * 255 / 31) for v in (0, 17, 6))   # RGB555 (0, 17, 6)
...
def title_screens():
    ...
    # 5. Seasons' red becomes Farore's green. Seasons is red and Ages is blue;
    # Tides, whose Oracle is Farore, is green (the human's call, S153). One
    # colour swapped for one colour: the ZELDA letters and the plaque are the
    # only pixels in that red, so every edge, highlight and accent stays the
    # source's. The green is a Game Boy Color value, RGB555 scaled by 255/31
    # the way every colour in this file is.
    for y in range(H):
        for x in range(W):
            if base[y][x] == RED:
                base[y][x] = GREEN
...
def main():
    ...
    lines = [
        ...
        '// and re-run it. See its header for what is extracted and the four',
        '// things done by rule.',
        '//',
